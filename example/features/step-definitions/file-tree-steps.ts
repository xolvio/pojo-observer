...
Given(/^the following files have been loaded by the File Tree$/, function (
  filesTable
) {
  fileTreeSteps.load(
    filesTable
      ...
      .map(
        (file) =>
          new File(file.filename, file.content, new Date(file.createdAt))
      )
  )
})

When(/^I select "([^"]*)" in the File Tree$/, function (filename) {
  fileTreeSteps.select(
    fileTreeSteps.files.find((file) => file.filename === filename)
  )
})
...
Then(/^the File Details should contain "([^"]*)"$/, function (createdAt) {
  expect(fileDetails.createdAt.toString()).toEqual(
